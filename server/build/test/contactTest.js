  ...
  describe('user send message', function () {
    ...
    describe('user sending messages', function () {
      ...
      it('should return bad request if required fields are not presents', function () {
        ...
        return _chai2.default.request(_app2.default).post('/api/v1/contacts').set('token', signedInUser.token).send(message3).then(function (res) {
      ...
      it('should not create a message for a sender that does not exist', function () {
        return _chai2.default.request(_app2.default).post('/api/v1/contacts').set('token', signedInUser.token).send(message4).then(function (res) {
          expect(res).to.have.status(400);
          expect(res.body).to.be.an('object');
        });
      });
      ...
      it('should not create a message if no message is posted', function () {
        return _chai2.default.request(_app2.default).post('/api/v1/contacts').set('token', signedInUser.token).send(message5).then(function (res) {
          expect(res).to.have.status(400);
          expect(res.body).to.be.an('object');
        });
      });
    ...
    describe('get messages', function () {
      it('should return all messages for the given userId', function () {
        return _chai2.default.request(_app2.default).get('/api/v1/contacts?userId=' + _signupTest.regularUser1.id).set('token', signedInUser.token).then(function (res) {
          expect(res).to.have.status(200);
