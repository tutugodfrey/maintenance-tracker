...
exports.default = describe('Users actions', function () {
  ...
  describe('get one request', function () {
    ...
    it('should return not found for requestId that does not exist for a logged in user', function () {
      ...
      return _chai2.default.request(app).get('/api/v1/users/requests/20?userId=' + userid).set('token', signedInUser.token).then(function (res) {
    ...
    it('should return not found for the requestId with no matching userId', function () {
      ...
      return _chai2.default.request(app).get('/api/v1/users/requests/' + id + '?userId=9').set('token', signedInUser.token).then(function (res) {
        expect(res).to.have.status(404);
        expect(res.body).to.be.an('object');
        expect(res.body).to.eql({ message: 'request not found' });
      });
  ...
  describe('Users request update', function () {
    ...
    it('should return not found for a request that does not exist', function () {
      ...
      return _chai2.default.request(app).put('/api/v1/users/requests/20?userId=' + userid).set('token', signedInUser.token).send({
        description: 'wall socket got burned and need replacement'
      }).then(function (res) {
        expect(res).to.have.status(404);
        expect(res.body).to.be.an('object');
  ...
  describe('delete request', function () {
    ...
    it('should return not found for a request that does not exist', function () {
      ...
      return _chai2.default.request(app).delete('/api/v1/users/requests/15?userId=' + userid).set('token', signedInUser.token).then(function (res) {
        expect(res).to.have.status(404);
        expect(res.body).to.be.an('object');
